  ...
  <el-container class="page-wrapper">
    <el-main>
      ...
      <div class="breadcrumb-containerParent">
        ...
        >
          <el-breadcrumb-item v-for="item in levelList" :to="item.path" v-bind:key="item.path">{{
            item.meta.title
          }}</el-breadcrumb-item>
        </el-breadcrumb>
        <span class="backBtn" v-show="showBackBtn" @click="goBack"
          ><i style="margin: 0;" class="el-breadcrumb__separator el-icon-arrow-left"></i>返回
        </span>
...
export default {
  ...
  watch: {
    $route(to) {
      console.log('vistor-roter-path======', to)
      if (to.path === '/index') {
      ...
      this.showBackBtn =
        this.$route.path === '/viewEntry' || this.$route.path === '/viewCommonEntry'
      this.getBreadcrumb()
      this.showHeaderFun()
    }
  },
  beforeRouteEnter(to, from, next) {
    next(vm => {
      ...
      vm.authFun()
      vm.showBackBtn = to.path === '/viewEntry' || to.path === '/viewCommonEntry'
    })
  },
  created() {
    this.getBreadcrumb()
...
.index-page {
  background: url('./index-bg.png') #fafafa 0 0 no-repeat;

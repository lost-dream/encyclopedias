  ...
  <div>
    ...
    <ul class="entryList">
      <li @click="seeEntry(item)" v-for="item in entryList">
        <img
          v-if="
            item.entrySummarys.length &&
              item.entrySummarys[0].summary &&
              JSON.parse(item.entrySummarys[0].summary).img
          "
          :src="baseUrlConfig.IMG_PREFIX + JSON.parse(item.entrySummarys[0].summary).img"
          alt=""
        />
        ...
        <div>
          ...
          <div
            v-if="item.entrySummarys.length && item.entrySummarys[0].summary"
            v-html="JSON.parse(item.entrySummarys[0].summary).text"
          ></div>
        </div>
      </li>
    </ul>
...
export default {
  ...
  created() {
    ...
    Cetc10Auth().init(function() {
      vm.specialId = sessionStorage.getItem('specialId')
      vm.specialInfo()
    })
  },
  mounted() {},
